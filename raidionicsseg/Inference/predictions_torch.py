...
def run_predictions(data: np.ndarray, model_path: str, parameters: ConfigResources) -> np.ndarray:
    ...
    """
    logging.debug("Loading torch model.")
        ...
        for x in range(0, int(np.ceil(data.shape[1] / (patch_size[0] - patch_offset[0])))):
            for y in range(0, int(np.ceil(data.shape[2] / (patch_size[1] - patch_offset[1])))):
                for z in range(0, int(np.ceil(data.shape[3] / (patch_size[2] - patch_offset[2])))):
                    ...
                    with torch.no_grad():
                        model_input_l = torch.from_numpy(model_input).float().to(device)
                        patch_pred_ts = model(model_input_l)
                    # Applying the softmax step, no performed by default in PyTorch
                    if not parameters.training_softmax_layer_included:
                        patch_pred = torch.softmax(patch_pred_ts.cpu(), 1).numpy()
                    else:
                        patch_pred = patch_pred_ts.cpu().numpy()
